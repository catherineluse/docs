...
In an RKE cluster, the cluster data is stored in etcd, a distributed database that runs on multiple controlplane nodes. By contrast, in a high-availability K3s cluster, the cluster data can be stored in an external MySQL datastore, allowing the controlplane nodes to be ephemeral.
