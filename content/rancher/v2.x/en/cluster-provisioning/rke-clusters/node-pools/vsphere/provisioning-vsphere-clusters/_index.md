...
For details on configuring the node template, refer to the [node template configuration reference.](#node-template-configuration-reference)
...
### Configure the vSphere Credential
...
### Configure Instances and Operating Systems
{{% tabs %}}
{{% tab "Rancher v2.3.3+" %}}
...
### If Not Already Enabled, Enable Disk UUIDs
...
### Optional: Configure Node Tags and Custom Attributes
...
7. {{< step_create-cluster_node-pools >}}
10. Optional for Rancher v2.3.0+: To make the node pool self-healing, enter a number greater than zero in the **Auto Replace** column. Rancher will use the node template for the given node pool to recreate the node if it becomes inactive for that number of minutes.
